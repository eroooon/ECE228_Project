...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 22:21:23 2021
...
class MidiNet(object):
    ...
    def build_model(self):
        ...
        self.prev_bar = tf.compat.v1.placeholder(tf.float32, [self.batch_size] + [self.output_w, self.output_h, self.c_dim],
                                    name='prev_bar')
            
        self.images = tf.compat.v1.placeholder(tf.float32, [self.batch_size] + [self.output_w, self.output_h, self.c_dim],
                                    name='real_images')
        self.sample_images= tf.compat.v1.placeholder(tf.float32, [self.sample_size] + [self.output_w, self.output_h, self.c_dim],
                                        name='sample_images')
